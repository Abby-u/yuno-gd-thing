...
function animx.newAnimation(params)
	...
	local img,img2,quads,sw,sh,startingFrame,delay,offset
...
function animx.newActor(arg)
	...
	if type(arg)=='string' then
		...
		local anims,offset=animx.newActorXML(img,removeExtension(arg,true)..'.xml')
...
function animx.newActorXML(image,filename)
	...
	for line in love.filesystem.lines(filename) do
		if i>1 and line:match('%a') and not line:match('<!') and line~="</TextureAtlas>" and line:match("<SubTexture") then
			...
			if not t[animName] then t[animName]={} a[animName]={} end
			...
			if not width then _,width = string.match(line, "w=([\"'])(.-)%1") end
			local _, height = string.match(line, "height=([\"'])(.-)%1")
			if not height then _,height = string.match(line, "h=([\"'])(.-)%1") end
			local _, offsetX = string.match(line, "frameX=([\"'])(.-)%1") offsetX = tonumber(offsetX) or 0
			local _, offsetY = string.match(line, "frameY=([\"'])(.-)%1") offsetY = tonumber(offsetY) or 0
			-- print(x,y,width,height,sw,sh,offsetX,offsetY)
			a[animName][frameNo]={offsetX,offsetY}
